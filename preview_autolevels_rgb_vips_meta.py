...
def save_image(
    ...
    *,
    ...
    - If meta_from is given, we *explicitly* copy metadata from that image
      right before saving.
    - extra_meta lets you add/override metadata items.

    JPEG quality used for .jpg/.jpeg.
    """
    out_path = str(path)
    ...
    if ext in [".jpg", ".jpeg"]:
        ...
        img_u8.write_to_file(out_path, Q=quality, strip=strip)
    else:
...
# We often want to preserve *all* metadata from the input file (EXIF/XMP/ICC/etc),
# plus any custom items you attach (eg. "xmp-data", "exif-ifd0-UserComment").
#
# libvips *usually* propagates metadata through pipelines automatically, but if you
# convert types / drop alpha / etc, it's easy to accidentally lose something.
# These helpers let you explicitly copy metadata at the end.
...
def apply_metadata(dst: pyvips.Image, meta: Mapping[str, Tuple[int, Any]], *, overwrite: bool = True) -> pyvips.Image:
    """
...
def encode_image(
    ...
    *,
    ...
    # Most foreign savers support strip=... (defaults to False).
    if ext in [".jpg", ".jpeg"]:
        return img_u8.write_to_buffer(ext, Q=quality, strip=strip)
    else:
...
def apply_final_pooled_rgb_stretch(
    ...
    v = clamp01(v)
    out = merge_alpha(v, a)
    dbg = {"pooled_rgb_q": q, "final_lo_bin": lo_bin, "final_hi_bin": hi_bin, "final_lo": lo, "final_hi": hi}
    return out, dbg
    ...
    *,
    ...
    if dump:
        ...
        for k in sorted(dbg.keys()):
            print(f"{k}: {dbg[k]}")
    save_image(out_path, out, quality=cfg.quality, meta_from=img)
