...
The \texttt{rasterizer} package turns a cloud of points (typically complex
numbers in the plane) into high-quality, bilevel (0/255) PNG images.  It is
organised into focused submodules, each handling a distinct stage of the
pipeline:

\begin{enumerate}
  \item \emph{Dot stamping} (\texttt{canvas.py}): generate small circular ``stamps'' and apply them
        at many point locations, in a Numba-accelerated loop.
  \item \emph{Radius bucketing} (\texttt{canvas.py}): group points by dot radius so that stamps can
        be reused efficiently.
  \item \emph{Format conversion} (\texttt{formats.py}): bridge between NumPy arrays and
        \texttt{pyvips} images.
  \item \emph{Composition} (\texttt{compose.py}): add labels, passepartout frames, and
        other visual effects.
  \item \emph{Output} (\texttt{export.py}): write PNG, JPEG, or VIPS files, and
        compose mosaics.
\end{enumerate}
...
\begin{itemize}
  \item \texttt{canvas.py} -- Point stamping, radius bucketing, coordinate projection, JIT warmup
  \item \texttt{formats.py} -- NumPy $\leftrightarrow$ PyVIPS conversions
  \item \texttt{compose.py} -- Footer labels, rounded passepartout, image padding
  \item \texttt{export.py} -- File output (PNG, JPEG, VIPS) and mosaic creation
  \item \texttt{raster.py} -- Re-exports all public functions for backwards compatibility
\end{itemize}

\subsection{Import Styles}

Direct submodule imports (preferred for new code):

\begin{tcolorbox}[enhanced, colback=black!2, colframe=black!50]
\begin{verbatim}
from rasterizer.canvas import stamp_points, bucket_by_radius
...
\begin{tcolorbox}[enhanced, colback=black!2, colframe=black!50]
\begin{verbatim}
from rasterizer.raster import stamp_points, save_png_bilevel
\end{verbatim}
\end{tcolorbox}

\newpage
\section{Dot Stamping (\texttt{canvas.py})}
...
\newpage
\section{Radius Bucketing (\texttt{canvas.py})}
...
\newpage
\section{Projection and Canvas (\texttt{canvas.py})}
...
\begin{itemize}
  \item \verb|np.max(np.abs(z))| finds the maximum distance from the origin
        in the point cloud.
  \item \verb|half| is this maximum distance, scaled by
...
\newpage
\section{NumPy \texorpdfstring{$\rightarrow$}{->} VIPS Bridge (\texttt{formats.py})}
...
\newpage
\section{Footer Labels (\texttt{compose.py})}
...
\newpage
\section{Rounded Passepartout (\texttt{compose.py})}
...
\newpage
\section{Saving PNGs and Mosaics (\texttt{export.py})}
...
\newpage
\section{JIT Warmup (\texttt{canvas.py})}

To avoid paying Numba compilation cost on the first "real" render, the
\texttt{canvas} module provides a lightweight warmup function:
