...
def save_image(
    ...
    *,
    ...
    if ext in [".jpg", ".jpeg"]:
        # strip alpha for jpeg (unless you really want it; JPEG can't store alpha)
        if img_u8.bands == 4:
            img_u8 = img_u8.extract_band(0, n=3)
        img_u8.write_to_file(out_path, Q=quality,subsample_mode="on")
    else:
        img_u8.write_to_file(out_path)
...
def apply_rgb_curve(
    ...
    if sigmoid_strength > 0:
        ...
        ss = 1.0 / (1.0 + (sigmoid_strength * (sigmoid_mid - v)).exp())
        v = (ss - s0) / denom_s
        v = clamp01(v)
    ...
    *,
    ...
    if dump:
        ...
        for k in sorted(dbg.keys()):
            print(f"{k}: {dbg[k]}")
    save_image(out_path, out, quality=cfg.quality)
